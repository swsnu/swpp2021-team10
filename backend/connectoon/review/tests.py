...
class ReviewTestCase(TestCase):
  def setUp(self):
    User = get_user_model()
    author = User.objects.create_user(email='dummy@user.com', password='1234', username='dummy')
    Work.objects.create(title = 'DummyTitle', year=2019, description="HI", link="https://www.naver.com/",
      completion=True, platform_id=1
    )
    Review.objects.create(work = Work.objects.first(), score = 0.0, title = "DUM", content="DUM_CONTENT", author = author)
